...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden transition-colors dark:bg-slate-950 dark:text-slate-100">
      ...
      <aside
        className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-slate-950 border-r border-slate-900 flex flex-col transition-all duration-200 ease-out md:transform-none dark:bg-slate-900 dark:border-slate-800/80 ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
      >
        {/* Brand */}
        <div className="h-16 flex items-center gap-2.5 px-5 border-b border-slate-900 shrink-0 dark:border-slate-800/60">
          ...
          <span className="text-base font-bold text-slate-100 tracking-tight dark:text-slate-100">
            Skillyon
          </span>
          <span className="text-[9px] font-semibold text-teal-400 bg-teal-950/40 px-1.5 py-0.5 rounded-full ml-auto tracking-wider uppercase dark:bg-teal-950/40 dark:text-teal-400">
            Parent
          </span>
          <button
            className="md:hidden ml-2 text-slate-400 hover:text-slate-200"
            onClick={() => setMobileMenuOpen(false)}
          >
            <X size={18} />
          </button>
          ...
          <div className="px-4 pt-4">
            <div className="relative">
              <select
                ...
                className="w-full appearance-none bg-slate-900 border border-slate-800 text-slate-100 text-sm font-medium rounded-lg px-3 py-2 pr-8 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-colors dark:bg-slate-800 dark:border-slate-700 dark:text-slate-200"
              >
                {children.map((c) => (
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium transition-colors ${
                  active
                    ? "bg-teal-950/40 text-teal-300 dark:bg-teal-950/40 dark:text-teal-400"
                    : "text-slate-400 hover:bg-slate-900 hover:text-slate-200 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200"
                }`}
              >
        ...
        {/* Footer */}
        <div className="px-3 py-3 border-t border-slate-900 dark:border-slate-800/80">
          <button
            onClick={() => {
              localStorage.removeItem("token");
              localStorage.removeItem("user");
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium text-slate-400 hover:bg-rose-950/20 hover:text-rose-400 transition-colors dark:text-slate-400 dark:hover:bg-rose-950/20 dark:hover:text-rose-400"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full">
        {/* Top bar — minimal */}
        <header className="h-14 bg-white border-b border-slate-100 flex items-center justify-between px-4 md:px-6 shrink-0 dark:bg-slate-900 dark:border-slate-800/80">
          <div className="flex items-center gap-3">
            <button
              className="md:hidden p-1.5 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-lg dark:text-slate-400 dark:hover:bg-slate-850"
              onClick={() => setMobileMenuOpen(true)}
            >
              <Menu size={20} />
            </button>
            <h1 className="text-sm font-semibold text-slate-900 hidden sm:block dark:text-slate-100">
              {t(
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              <button
                ...
                className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors relative dark:hover:bg-slate-850 cursor-pointer flex items-center justify-center"
              >
                <Bell size={18} />
            ...
            <div className="flex items-center gap-2 pl-1">
              ...
              <span className="text-sm font-medium text-gray-700 hidden sm:block dark:text-slate-300">
                {displayName}
              </span>
            </div>
          </div>
