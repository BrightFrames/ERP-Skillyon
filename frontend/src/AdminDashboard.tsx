  ...
  return (
    <div className="flex flex-col gap-8 text-slate-900 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Here's what's happening at Skillyon today.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-card p-6 rounded-3xl border border-border shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.1)] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between h-40 group relative overflow-hidden">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200/60 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.1)] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between h-40 group relative overflow-hidden">
          ...
          <p className="text-sm font-semibold text-muted-foreground z-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="flex items-center gap-4">
              {userRole === "ADMIN" && (
                <button
                  ...
                  className="flex items-center gap-1.5 text-sm font-bold text-white bg-linear-to-r from-indigo-600 to-purple-600 px-4 py-2 rounded-xl hover:shadow-lg hover:shadow-indigo-500/30 hover:-translate-y-0.5 transition-all"
                >
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-[#3b3dbf] p-6 rounded-2xl shadow-lg text-white flex-1 relative overflow-hidden">
            ...
            <h3 className="font-bold text-base mb-5 relative z-10">
              Quick Actions
            </h3>
        ...
        <div className="w-full overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="bg-white text-slate-400 font-semibold border-b border-slate-100">
              <tr>
                <th className="px-7 py-4 font-bold text-xs uppercase tracking-wider">
                  Student Name
                </th>
                <th className="px-7 py-4 font-bold text-xs uppercase tracking-wider">
                  ID
                ...
                <th className="px-7 py-4 font-bold text-xs uppercase tracking-wider text-right">
                  Action
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50 font-medium">
              {loading ? (
                <tr>
                  <td
                    colSpan={6}
                    className="px-7 py-10 text-center text-slate-400 text-sm"
                  >
                    Loading students...
                  </td>
                </tr>
                ...
                recentStudents.map((s) => (
                  ...
                  >
                    <td className="px-7 py-4">
                      <div className="flex items-center gap-4">
                        {s.avatar ? (
                          ...
                          <div className="w-10 h-10 rounded-full bg-linear-to-br from-indigo-100 to-purple-100 text-indigo-700 flex items-center justify-center text-sm font-bold shrink-0 border-2 border-white shadow-sm">
                            {getInitials(s.name)}
                          </div>
                        )}
                    ...
                    <td className="px-7 py-4 text-slate-400 text-xs font-bold">
                      #{s.id.toString().padStart(4, "0")}
                    </td>
                    <td className="px-7 py-4">
                      {s.class_name ? (
                        <span className="px-3 py-1 bg-indigo-50/80 text-indigo-700 rounded-lg text-xs font-bold border border-indigo-100/50">
                          {s.class_name}
                        </span>
                      ) : (
                    ...
                    <td className="px-7 py-4 text-xs text-slate-500 font-bold">
                      {s.gender || "—"}
                    </td>
                    <td className="px-7 py-4">
                      <span
                        className={`px-3 py-1 rounded-lg text-[10px] font-extrabold tracking-wide uppercase ${
                          s.status === "Active"
                    ...
                    <td className="px-7 py-4 text-right">
                      <Link
                        to={`/students/${s.id}`}
                        className="text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 p-2.5 rounded-xl transition-all inline-flex border border-transparent hover:border-indigo-100"
                      >
                        <Eye size={18} />
                      </Link>
                    </td>
            ...
            <form onSubmit={handleAddClass} className="p-7 flex flex-col gap-5">
              ...
              <div className="flex justify-end gap-3 mt-4">
                ...
                <button
                  type="submit"
                  disabled={
                    isSubmittingClass || !newGrade.trim() || !newSection.trim()
                  }
                  className="px-6 py-3 rounded-xl text-sm font-bold text-white bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 disabled:opacity-50 transition-all shadow-md hover:shadow-lg flex items-center gap-2"
                >
                  {isSubmittingClass ? (
                    "Creating..."
                  ) : (
