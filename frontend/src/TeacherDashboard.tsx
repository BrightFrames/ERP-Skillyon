...
export default function TeacherDashboard() {
  ...
  const totalGrades = Object.values(gradebooks).reduce(
    (sum, gb) => sum + (gb.grades?.length || 0),
    0,
  );
  ...
  Object.entries(gradebooks).forEach(([classId, gb]) => {
    ...
    gb.students?.forEach((s: any) => {
      ...
      const avgScore =
        graded.reduce((a: number, g: any) => a + g.score, 0) / graded.length;
      const maxScore =
        gb.assessments?.reduce(
          (a: number, a2: any) => a + (a2.max_score || 100),
          0,
        ) / (gb.assessments?.length || 1) || 100;
      if ((avgScore / maxScore) * 100 < 60) {
        atRiskStudents.push({
          ...s,
          className: cls?.name,
          avgScore: Math.round((avgScore / maxScore) * 100),
        });
      }
    });
  });
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            Subject: <span className="text-primary font-bold">{subject}</span>
          </p>
        </div>
        <Link
          to="/classes"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            sub: ungradedSlots > 0 ? "need grading" : "all caught up!",
          },
        ].map((item, i) => (
          <div
            key={i}
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-2 flex-1 overflow-y-auto">
              {atRiskStudents.slice(0, 6).map((s, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 p-2.5 rounded-xl bg-red-50/50 border border-red-100"
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm p-6">
          ...
          <div className="bg-indigo-50/60 border-l-4 border-[#3b3dbf] p-4 rounded-r-xl mb-4">
            <p className="text-xs font-bold text-[#3b3dbf] mb-0.5">
              {subject} — Today
            </p>
            <p className="font-bold text-zinc-800 text-sm">
              Review & Assessment Prep
            </p>
          </div>
          <div className="space-y-3">
            {[
              "Review previous assessments with class",
              "Cover any at-risk student topics",
              "Distribute marks & feedback",
            ].map((item, i) => (
              <div key={i} className="flex items-start gap-2.5">
                {i === 0 ? (
                  <CheckCircle2
                    size={16}
                    className="text-[#3b3dbf] shrink-0 mt-0.5"
                  />
                ) : (
          ...
          <div className="relative z-10">
            ...
            <div className="flex gap-2">
              {classes.slice(0, 3).map((cls) => (
                <span
                  key={cls.id}
                  className="px-2.5 py-1 bg-white/20 text-white text-[10px] font-bold rounded-full"
                >
                  {cls.name}
                </span>
              ))}
