...
export function applySettings(appearance: AppearanceSettings) {
  ...
  // 4. Dispatch event for reactive updates in React components (e.g. language translation)
...
export async function fetchAndApplySettings() {
  try {
    ...
    if (res.data && res.data.appearance && Object.keys(res.data.appearance).length > 0) {
